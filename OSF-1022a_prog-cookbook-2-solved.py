...
BOOK_1_CHECKLIST = \
[
    [True], "IPO - Input, Process, Output Cycle",
    [True], ("How to get user input", input),
    [True], ("How to get program output", print),
...
def allow_i(chosen, i, n):
    return None if len(chosen) >= n else i not in chosen
...
def random_names_i():
    ...
    for s in range(n):
        while True:
            i = randrange(n)
            should_select = allow_i(i_s, i, len(names))
            if should_select is True:
                chosen_names = chosen_names + [names[i]]
                i_s = i_s + [i]
                break
            elif should_select is None:
                break
            else:
                pass
    print_list(chosen_names)
